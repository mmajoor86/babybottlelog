...
def app():
    ...
    activity_count = df_filtered.groupby(['Date','Activity']).size().reset_index(name='Count')
    fig_activity = px.bar(activity_count, x='Date', y='Count', color='Activity', barmode='stack', title='Activity Count Over Time', labels={'Date':'Date', 'Count':'Activity Count'})
    ...
    if 'Amount Consumed' in df.columns:
        ...
        fig_amount = px.bar(amount_consumed, x='Date', y='Amount Consumed', title='Amount Consumed Over Time')
...
def read_files(datadir:str='data')-> pd.DataFrame:
    ...
    if len(csv_files) == 0:
        st.write("No data available yet. Please log some activities in the Data Entry page. 🍼👶")
