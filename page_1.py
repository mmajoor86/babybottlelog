...
def app():
    ...
    amount = st.number_input("Amount Consumed (ml)", min_value=0, max_value=200, step=10, format="%d", value=0)

    # Submit button
    if st.button("Submit"):
        st.write(f"Date-Time: {date_time}")
        st.write(f"Activity: {activity}")
        st.write(f"Amount Consumed: {amount} ml" if amount > 0 else "Amount Consumed: N/A")
        store_data_csv(date_time, activity, amount)
        st.markdown("### Updated Data 📊")



def store_data_csv(date_time:datetime,
                   activity: str,
                   amount: int) -> pd.DataFrame:
    formatted_date_time = date_time.strftime("%Y-%m-%d_%H-%M-%S")
    """Store the input data as a csv file"""
    data = { 'Date-Time': [date_time], 'Activity': [activity], 
